...
// src/http/controllers/check-ins/create-check-in-controller.spec.ts
...
describe("Create Check-in (e2e)", () => {
  ...
  it("should be able to create a check-in", async () => {
    ...
    const response = await (0, import_supertest2.default)(app.server).post(`/gyms/${gym.id}/check-ins`).set("Authorization", `Bearer ${token}`).send({
      latitude: -27.2092052,
      longitude: -49.6401091
    });
