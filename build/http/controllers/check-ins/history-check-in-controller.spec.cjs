...
// src/http/controllers/check-ins/history-check-in-controller.spec.ts
...
describe("Check-in History (e2e)", () => {
  ...
  it("should be able to list the history of check-ins", async () => {
    ...
    const response = await (0, import_supertest2.default)(app.server).get("/check-ins/history").set("Authorization", `Bearer ${token}`).send();
    globalExpect(response.statusCode).toEqual(200);
    globalExpect(response.body.checkIns).toEqual([
