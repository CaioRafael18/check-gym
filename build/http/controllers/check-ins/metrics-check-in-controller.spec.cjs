...
// src/http/controllers/check-ins/metrics-check-in-controller.spec.ts
...
describe("Check-in Metrics (e2e)", () => {
  ...
  it("should be able to get the total count of check-ins", async () => {
    ...
    const response = await (0, import_supertest2.default)(app.server).get("/check-ins/metrics").set("Authorization", `Bearer ${token}`).send();
    globalExpect(response.statusCode).toEqual(200);
    globalExpect(response.body.checkInsCount).toEqual(2);
  });
});
