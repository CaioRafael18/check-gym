...
// src/http/controllers/check-ins/validate-check-in-controller.spec.ts
...
describe("Validate Check-in (e2e)", () => {
  ...
  it("should be able to validate a check-in", async () => {
    ...
    const response = await (0, import_supertest2.default)(app.server).patch(`/check-ins/${checkIn.id}/validate`).set("Authorization", `Bearer ${token}`).send();
    globalExpect(response.statusCode).toEqual(204);
    checkIn = await prisma.checkIn.findUniqueOrThrow({
      where: {
        id: checkIn.id
      }
    });
    globalExpect(checkIn.validatedAt).toEqual(globalExpect.any(Date));
  });
});
