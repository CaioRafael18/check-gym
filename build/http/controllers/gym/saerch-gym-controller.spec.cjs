...
// src/http/controllers/gym/saerch-gym-controller.spec.ts
var import_supertest2 = __toESM(require("supertest"), 1);
...
// node_modules/.pnpm/vitest@3.2.4_@types+node@24.0.15_jiti@2.4.2_tsx@4.20.3/node_modules/vitest/dist/chunks/utils.XdZDrNZV.js
...
// node_modules/.pnpm/vitest@3.2.4_@types+node@24.0.15_jiti@2.4.2_tsx@4.20.3/node_modules/vitest/dist/chunks/_commonjsHelpers.BFTU3MAI.js
...
// node_modules/.pnpm/vitest@3.2.4_@types+node@24.0.15_jiti@2.4.2_tsx@4.20.3/node_modules/vitest/dist/chunks/date.Bq6ZW5rf.js
var RealDate = Date;
var now2 = null;
...
// node_modules/.pnpm/vitest@3.2.4_@types+node@24.0.15_jiti@2.4.2_tsx@4.20.3/node_modules/vitest/dist/chunks/vi.bdSIJ99Y.js
var unsupported = [
  "matchSnapshot",
  "toMatchSnapshot",
...
// node_modules/.pnpm/vitest@3.2.4_@types+node@24.0.15_jiti@2.4.2_tsx@4.20.3/node_modules/vitest/dist/index.js
...
function verifyUserRole(roleToVerify) {
  return async (request3, reply) => {
    const { role } = request3.user;
...
async function create(request3, reply) {
  ...
  const { gymId } = createCheckInParamsSchema.parse(request3.params);
  const { latitude, longitude } = createCheckInBodySchema.parse(request3.body);
...
async function metrics(request3, reply) {
...
// src/http/controllers/check-ins/validate-check-in-controller.ts
async function validate(request3, reply) {
  const validateCheckInParamsSchema = import_zod4.z.object({
    checkInId: import_zod4.z.string().uuid()
  });
  const { checkInId } = validateCheckInParamsSchema.parse(request3.params);
...
async function create2(request3, reply) {
  ...
  const { title, description, phone, latitude, longitude } = createGymBodySchema.parse(request3.body);
...
// src/http/controllers/gym/nearby-gym-controller.ts
async function nearby(request3, reply) {
  ...
  const { latitude, longitude } = nearbyGymsQuerySchema.parse(request3.query);
...
async function Authenticate(request3, reply) {
  ...
  const { email, password } = registerBodySchema.parse(request3.body);
  try {
...
async function register(request3, reply) {
  ...
  const { name, email, password } = registerBodySchema.parse(request3.body);
  try {
...
async function createAndAuthenticateUser(app2, isAdmin = false) {
  await prisma.user.create({
    data: {
      name: "John Doe",
      email: "johndoe@example.com",
      passwordHash: await (0, import_bcryptjs3.hash)("123456", 6),
...
describe("Search Gyms (e2e)", () => {
  ...
  it("should be able to search gyms by title", async () => {
    ...
    await (0, import_supertest2.default)(app.server).post("/gyms").set("Authorization", `Bearer ${token}`).send({
    ...
    const response = await (0, import_supertest2.default)(app.server).get("/gyms/search").query({
      q: "JavaScript"
    }).set("Authorization", `Bearer ${token}`).send();
    globalExpect(response.statusCode).toEqual(200);
    globalExpect(response.body.gyms).toHaveLength(1);
    globalExpect(response.body.gyms).toEqual([
