...
// src/http/controllers/users/profile-controller.spec.ts
...
describe("Profile (e2e)", () => {
  ...
  it("should be able to get user profile", async () => {
    ...
    const profileResponse = await (0, import_supertest2.default)(app.server).get("/me").set("Authorization", `Bearer ${token}`).send();
    globalExpect(profileResponse.statusCode).toEqual(200);
    globalExpect(profileResponse.body.user).toEqual(
