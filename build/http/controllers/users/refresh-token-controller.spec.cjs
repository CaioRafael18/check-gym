...
// src/http/controllers/users/refresh-token-controller.spec.ts
...
async function verifyJwt(request2, reply) {
  try {
...
async function create(request2, reply) {
  ...
  const { gymId } = createCheckInParamsSchema.parse(request2.params);
  const { latitude, longitude } = createCheckInBodySchema.parse(request2.body);
  const checkInUseCase = makeCheckInUseCase();
  await checkInUseCase.execute({
    gymId,
    userId: request2.user.sub,
...
async function metrics(request2, reply) {
...
// src/http/controllers/check-ins/validate-check-in-controller.ts
async function validate(request2, reply) {
  const validateCheckInParamsSchema = import_zod4.z.object({
    checkInId: import_zod4.z.string().uuid()
  });
  const { checkInId } = validateCheckInParamsSchema.parse(request2.params);
...
async function create2(request2, reply) {
  ...
  const { title, description, phone, latitude, longitude } = createGymBodySchema.parse(request2.body);
...
// src/http/controllers/gym/nearby-gym-controller.ts
async function nearby(request2, reply) {
  ...
  const { latitude, longitude } = nearbyGymsQuerySchema.parse(request2.query);
...
// src/http/controllers/users/authenticate-controller.ts
async function Authenticate(request2, reply) {
...
async function profile(request2, reply) {
...
async function refresh(request2, reply) {
  await request2.jwtVerify({ onlyCookie: true });
  const { role } = request2.user;
  const token = await reply.jwtSign(
    { role },
    {
      sign: {
        sub: request2.user.sub
      }
    }
  );
  const refreshToken = await reply.jwtSign(
...
describe("Refresh Token (e2e)", () => {
  ...
  it("should be able to refresh a token", async () => {
    ...
    const response = await (0, import_supertest.default)(app.server).patch("/token/refresh").set("Cookie", cookies).send();
    globalExpect(response.status).toEqual(200);
    globalExpect(response.body).toEqual({
      token: globalExpect.any(String)
    });
    globalExpect(response.get("Set-Cookie")).toEqual([globalExpect.stringContaining("refreshToken=")]);
  });
});
