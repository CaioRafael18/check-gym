...
// src/use-cases/check-in-use-case.spec.ts
...
var InMemoryCheckInsRepository = class {
  ...
  async findByUserIdOnDate(userId, date) {
    ...
    const checkInOnSameDate = this.items.find((checkIn) => {
      const checkInDate = (0, import_dayjs.default)(checkIn.createdAt);
...
describe("Check-in Use Case", () => {
  ...
  it("should not be able to check in on distant gym", async () => {
    gymsRepository.items.push({
      ...
      description: "",
      phone: "",
      latitude: new import_library.Decimal(-27.0747279),
      longitude: new import_library.Decimal(-49.4889672)
    });
    await globalExpect(
      () => sut.execute({
        gymId: "gym-02",
